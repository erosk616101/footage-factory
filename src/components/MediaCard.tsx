      ...
      <div className={`relative ${getAspectRatio()} overflow-hidden`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="flex items-center space-x-2">
            <button 
              className="w-8 h-8 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-gray-800 hover:text-primary transition-colors"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                toast({
          ...
          <button 
            className="bg-primary text-white text-xs font-medium rounded-full px-3 py-1 flex items-center space-x-1 hover:bg-primary/90 transition-colors"
